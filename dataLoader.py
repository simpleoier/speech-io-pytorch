...
def default_collate(batch_data):
    ...
    """batch_data = [inputs_batch, targets_batch] where inputs_batch and targets_batch could be None """
    for data in batch_data:     # inputs or targets
        if not data: continue
        for batch in data:      # multiple inputs or multiple targets
...
class HTKDataLoaderIter(object):
    ...
    def __next__(self):
        if self.num_workers == 0:   # same_process loading
            ...
            indices, lengths = torch.LongTensor(self._next_batch_indices())
            ...
            for i in range(len(self.block_data)):
                ...
                for j in range(len(self.block_data[i])):
                    batch[i].append(self.block_data[i][j][indices])
    ...
    def _get_SCP_block(self, subdataset):
        ...
        if self.frame_mode:
            random_block_data = defaultTensor(self.random_samples_remaining, dimension).zero_()
        else:
            random_block_data = defaultTensor(len(self.random_block_keys), self.max_utt_len, dimension).zero_()
        ...
        for i in range(len(self.random_block_keys)):
            ...
            if self.frame_mode:
                random_block_data[start_cnt:start_cnt+feat_len] = torch.from_numpy(tmp_data)
                start_cnt += feat_len
            else:
                random_block_data[i][0:feat_len].copy_(torch.from_numpy(tmp_data))
    ...
    def _get_MLF_block(self, subdataset):
        ...
        if self.frame_mode:
            random_block_data = defaultTensor(self.random_samples_remaining).zero_()
        else:
            random_block_data = defaultTensor(len(self.random_block_keys), self.max_utt_len).zero_()
        ...
        for i in range(len(self.random_block_keys)):
            ...
            if self.frame_mode:
                random_block_data[start_cnt:start_cnt+feat_len] = defaultTensor(tmp_data)
                start_cnt += feat_len
            else:
                random_block_data[i][:len(tmp_data)].copy_(defaultTensor(tmp_data))
        return torch.IntTensor(random_block_data)
...
class HTKDataLoader(DataLoader):
    ...
    def __init__(self, dataset, batch_size=1, num_workers=0,
        ...
        #if self.frame_mode:
        #    self.sampler = FrameLevSampler
        #else:
        #    self.sampler = UtteranceLevSampler
