...
import numpy as np
import os, re, gzip, struct, subprocess
...
if not 'KALDI_ROOT' in os.environ:
    ...
    os.environ['KALDI_ROOT'] = '/speechlab/tools/KALDI/kaldi-master'

# Add kaldi tools to path
os.environ['PATH'] = os.popen('echo $KALDI_ROOT/src/bin:$KALDI_ROOT/tools/openfst/bin:'
                              '$KALDI_ROOT/src/fstbin/:$KALDI_ROOT/src/gmmbin/:'
                              '$KALDI_ROOT/src/featbin/:$KALDI_ROOT/src/lm/:'
                              '$KALDI_ROOT/src/sgmmbin/:$KALDI_ROOT/src/sgmm2bin/:'
                              '$KALDI_ROOT/src/fgmmbin/:$KALDI_ROOT/src/latbin/:'
                              '$KALDI_ROOT/src/nnetbin:$KALDI_ROOT/src/nnet2bin:'
                              '$KALDI_ROOT/src/nnet3bin:$KALDI_ROOT/src/online2bin/:'
                              '$KALDI_ROOT/src/ivectorbin/:$KALDI_ROOT/src/lmbin/').readline().strip() + ':' + os.environ['PATH']
...
def open_or_fd(file, mode='rb'):
    ...
    try:
        ...
        # input pipe?
            ...
            fd = subprocess.Popen(file[:-1], shell=True, stdout=subprocess.PIPE).stdout
            #fd = os.popen(file[:-1], 'rb')
        # output pipe?
        elif file[0] == '|':
            fd = subprocess.Popen(file[1:], shell=True, stdin=subprocess.PIPE).stdin
...
def _read_mat_binary(fd):
    ...
    elif sample_size == 8: vec = np.frombuffer(buf, dtype='float32')
    else: raise BadSampleSize
...
def _read_mat_ascii(fd):
    ...
    while 1:
        ...
        if arr[-1] != ']':
            ...
            row.append(np.array(arr[:-1], dtype='float32')) # last line
            mat = np.vstack(row)
            return mat
...
def read_post(file_or_fd):
    ...
    for i in range(outer_vec_size):
        ...
        for j in range(inner_vec_size):
            ...
            post[j] = struct.unpack('<i', fd.read(4))[0]    # post
